...
| short_vectors_form / closest_vectors_form returning Vec | return lattice::Enumeration; take `visit_limit: u64` | pending | AUDIT R1/R3: pruning is certified by exact Gram–Schmidt and interval bounds, and the result says whether the search was exhausted, stopped at its visit limit, or stopped because doubles could not bound a branch; every returned vector is exactly rechecked either way. A negative bound is an empty, exhausted search for both (no panic). |
| — | lattice::Enumeration | pending | the vectors found, nearest first, with `visits()` and `outcome()`; fields private. |
| — | lattice::EnumerationOutcome { Exhausted, VisitLimit, NumericalLimit } | pending | non-exhaustive Copy enum; only `Exhausted` certifies that no vector within the bound was missed. |
