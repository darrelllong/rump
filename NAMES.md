...
| — | modular::mod_inverse_u128 | pending | the double-word sibling of `mod_inverse_u64`, total over every `u128` modulus (unsigned cofactors with a tracked sign, so no `i128` ceiling at 2¹²⁷); `None` when the gcd exceeds one, panics on a zero modulus. Owner: factoring's ECM, whose `mod_inverse128` it replaces. |
| — | number_theory::crt_combine_u64 | pending | two congruences with word moduli combined into the residue below their product, which always fits `u128`: `crt_combine`'s contract (residues may be unreduced; `None` for a zero or non-coprime modulus) without the heap. Owner: factoring's lattice sieve, whose `kleinjung::crt` it replaces. |
