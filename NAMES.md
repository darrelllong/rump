...
| gf2::block_lanczos_dependencies takes `threads` | both sieves | done — bounded workers are retained for the solve, applies fan out only when each worker gets thousands of folds, and results are bit-identical at any count, tested above the parallel threshold |
...
| — | modular::mod_inverse_u128 | done | the double-word sibling of `mod_inverse_u64`, total over every `u128` modulus (unsigned cofactors with a tracked sign, so no `i128` ceiling at 2¹²⁷); `None` when the gcd exceeds one, panics on a zero modulus. Owner: factoring's ECM, whose `mod_inverse128` it replaces. |
| — | number_theory::crt_combine_u64 | done | two congruences with word moduli combined into the residue below their product, which always fits `u128`: `crt_combine`'s contract (residues may be unreduced; `None` for a zero or non-coprime modulus) without the heap. Owner: factoring's lattice sieve, whose `kleinjung::crt` it replaces. |
| short_vectors_form / closest_vectors_form returning Vec | return lattice::Enumeration; take `visit_limit: u64` | done | pruning is certified by exact Gram–Schmidt and interval bounds, and the result says whether the search was exhausted, stopped at its visit limit, or stopped because doubles could not bound a branch; every returned vector is exactly rechecked either way. A negative bound is an empty, exhausted search for both (no panic). |
| — | lattice::Enumeration | done | the vectors found, nearest first, with `visits()` and `outcome()`; fields private. |
| — | lattice::EnumerationOutcome { Exhausted, VisitLimit, NumericalLimit } | done | non-exhaustive Copy enum; only `Exhausted` certifies that no vector within the bound was missed. |
| number_theory::regularized_incomplete_beta returning f64 | return Result<f64, number_theory::NumericalError> | done | the continued fraction's last iterate was returned whether or not it converged, and the normalisation cancelled, so `I_(1/2)(10¹⁰, 10¹⁰)` came out −5.78. A non-converged or out-of-range evaluation, or an argument outside the domain, is now an error. |
| number_theory::student_t_quantile returning f64 and panicking | return Result<f64, number_theory::NumericalError> | done | built on the incomplete beta, so a numerical failure reaches the caller; zero degrees of freedom or a probability outside (½, 1) is `Domain`, not a panic. Owner: factoring's polynomial race, which must keep its contenders on an error. |
| — | number_theory::NumericalError { Domain, NotConverged } | done | non-exhaustive Copy enum with Display and Error: an argument outside the function's domain, or an evaluation that could not reach its stated accuracy. |
| gfp::kernel_vector returning Option<Vec<BigUint>> | return gfp::Kernel { Vector, NoKernel, Inconclusive } | done | `None` conflated two answers a caller must act on differently: a draw that failed, which wants another draw, and a matrix with no kernel, which wants different rows. factoring-5d hit exactly that, retrying draws against a matrix that needed more relations (2026-09-17). `NoKernel` is certain when the recurrence reached the matrix's dimension and otherwise carries the same `n/l` risk as every other answer. |
| — | BigUint::mul_into, BigUint::keep_low_bits | done | the product's three-operand form and `self mod 2^k` in place. A field layer built on `BigUint` made six allocations per multiply — the product, the quotient and remainder of the reduction, and the temporaries between — and that, not limb arithmetic, was the whole of its gap to a C++ layer. With these two and the in-place shifts and `+=`/`-=` a Mersenne fold allocates nothing; `tests/allocation_free_fold.rs` counts the allocator's calls across a thousand folds and requires zero. Owner: secret-sharing's field layer (2026-09-23). |
| — | gfp::{Field, SparseMatrix, kernel_vector, minimal_polynomial} | done | linear algebra over a large prime field: a sparse matrix whose entries are small integers, held with its `±1` entries in their own lists, and Wiedemann's algorithm for a kernel vector. The homogeneous system an index-calculus discrete logarithm ends in. Scalar, not blocked: a blocked form distributes and is what the sizes past ~110 digits need, but nothing is built on this yet. Owner: factoring's discrete logarithms (2026-09-17). |
| number_theory::ln_gamma / regularized_incomplete_beta / student_t_quantile / NumericalError | removed | done | floating statistical kernels, not arithmetic: moved unchanged to entropy's always-compiled `entropy::math` (entropy 0.6.0, `b8975c1`), with their reference scripts and fixtures. factoring, their only caller, switched in `e5848c1` and pins entropy there; cryptography never called them. rump keeps no forwarding wrapper — a rump→entropy dependency would close a cycle. |
| — | modular::MontgomeryContext::gcd_with_modulus | done | `gcd(v, n)` for the value a residue encodes, from the encoded limbs: `n` is odd, so `R` is a unit and `gcd(v·R mod n, n) = gcd(v, n)`; no decoding, representation stays opaque; `ContextMismatch` for a foreign residue. Owner: factoring's rho, whose batch product and replay each decoded before the gcd (2026-09-17). |
