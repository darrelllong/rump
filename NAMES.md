...
| number_theory::regularized_incomplete_beta returning f64 | return Result<f64, number_theory::NumericalError> | pending | AUDIT R1 (2026-09-16 review of 66651ab): the continued fraction's last iterate was returned whether or not it converged, and the normalisation cancelled, so `I_(1/2)(10¹⁰, 10¹⁰)` came out −5.78. A non-converged or out-of-range evaluation, or an argument outside the domain, is now an error. |
| number_theory::student_t_quantile returning f64 and panicking | return Result<f64, number_theory::NumericalError> | pending | built on the incomplete beta, so a numerical failure reaches the caller; zero degrees of freedom or a probability outside (½, 1) is `Domain`, not a panic. Owner: factoring's polynomial race, which must keep its contenders on an error. |
| — | number_theory::NumericalError { Domain, NotConverged } | pending | non-exhaustive Copy enum with Display and Error: an argument outside the function's domain, or an evaluation that could not reach its stated accuracy. |
