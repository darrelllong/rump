...
| Exact NTT multiplication and squaring | `bigint::ntt::multiply` / `bigint::ntt::square` (dispatched from `BigUint::mul` / `square`) | Pollard, *The Fast Fourier Transform in a Finite Field*, Math. Comp. 25 (1971), 365–374, the small-prime (finite-field) NTT; iterative radix-2 transform after Cooley & Tukey, *An Algorithm for the Machine Calculation of Complex Fourier Series*, Math. Comp. 19 (1965), 297–301. Two NTT primes with exact CRT recovery; the transform length is fixed-capped by the primes' 2-adic valuation, not the recursive Schönhage–Strassen scheme over Z/(2^n+1). Squaring pointwise squares one transformed operand. |
...
| Structured elimination with composition replay | `filter_merge`, `FilteredMatrix`, `SparseMatrix` (`src/gf2/filter.rs`) | Structured Gaussian elimination as NFS filtering practice: Cavallar, *Strategies in filtering in the number field sieve*, ANTS-IV, LNCS 1838 (2000), 209–231; modern treatment and the minimum-spanning-tree pivot (least fill-in over a star elimination) in Bouillaguet & Zimmermann, *Parallel structured Gaussian elimination for the number field sieve*, Mathematical Cryptology 1 (2021), 22–39, §3. Eliminations ordered by fill after Markowitz, *The elimination form of the inverse and its application to linear programming*, Management Science 3 (1957), 255–269; the stopping rule (merge while fill × (rows − 1) < nonzeros, the descent of Block Lanczos's rows · nonzeros) is this crate's derivation. Replay by carried row compositions rather than a history log is this crate's choice, proven by the expansion identity in tests. |
| Minimum spanning tree of a column's members | `plan` (`src/gf2/filter.rs`) | Jarník, *O jistém problému minimálním*, Práce Moravské přírodovědecké společnosti 6 (1930), 57–63; rediscovered by Prim, *Shortest connection networks and some generalizations*, Bell System Tech. J. 36 (1957), 1389–1401. |
| Clique removal of excess rows | `Filter::purge` (`src/gf2/filter.rs`) | Cavallar, *Strategies in filtering in the number field sieve*, ANTS-IV, LNCS 1838 (2000), 209–231: rows bound through weight-two columns are removed as a unit, heaviest first, until the excess reaches its target. |
| Square root in a finite field | `PolyMod::sqrt_in_field` | Tonelli, *Bemerkung über die Auflösung quadratischer Congruenzen*, Göttinger Nachrichten (1891); Shanks, *Five number-theoretic algorithms*, Proc. 2nd Manitoba Conf. on Numerical Mathematics (1972), 51–70 — the classical algorithm over `𝔽_p`, carried into `𝔽_{q^d}` unchanged, with the `(q^d+1)/4` shortcut taken when `q^d ≡ 3 (mod 4)`. |
...
| Square roots in `ℤ[x]/(f)` modulo prime powers (p-adic Newton) | `HenselSquareRoot` (`src/poly/hensel_sqrt.rs`) | Hensel, *Neue Grundlagen der Arithmetik*, J. reine angew. Math. 127 (1904), 51–84, in the polynomial form of Cohen, §3.5.3; the precision-doubling schedule and the companion reciprocal iteration are Brent, *Fast multiple-precision evaluation of elementary functions*, J. ACM 23(2) (1976), 242–251, §§3–4; the half-width correction is the p-adic form of Karp & Markstein, *High-precision division and square root*, ACM TOMS 23(4) (1997), 561–589. |
...
| Algorithm | Location | Reference |
|---|---|---|
| LLL reduction (integral) | `lll_reduce`, `lll_reduce_delta`, `lll_reduce_form` (the same algorithm under an integral quadratic form) | Lenstra, Lenstra & Lovász, *Factoring polynomials with rational coefficients*, Math. Ann. 261 (1982), 515–534; integral form Cohen, *A Course in Computational Algebraic Number Theory*, Algorithm 2.6.3; reduction under the `L²`-integral form of a polynomial's coefficients after Herrmann, May & Ritzenhofen, *Polynomial selection using lattices*, Factoring 2009 workshop (Bochum), slides. |
| Log-gamma | `ln_gamma` | Lanczos, *A precision approximation of the gamma function*, J. SIAM Numer. Anal. B 1 (1964), 86–96, with `g = 7` and nine coefficients chosen to make the approximation exact at `Γ(1), …, Γ(9)` (derived and checked by `scripts/lanczos_coefficients.py`); below `1/2`, the recurrence `ln Γ(x) = ln Γ(1 + x) − ln x`, DLMF §5.5(i), eq. 5.5.1; from `10⁷`, Stirling's series, DLMF 5.11.1, with the remainder bound of §5.11(ii). |
| Regularised incomplete beta | `regularized_incomplete_beta` | Continued fraction DLMF 8.17.22 with the symmetry 8.17.4, by the modified Lentz method (Lentz, Applied Optics 15 (1976), 668–671; Thompson & Barnett, J. Comput. Phys. 64 (1986), 490–509) in double-double arithmetic (Dekker, Numer. Math. 18 (1971), 224–242); the prefactor by the rearrangement of DiDonato & Morris, ACM TOMS 18 (1992), 360–373, with Stirling's remainder from DLMF 5.11.1; references from `scripts/incomplete_beta_reference.py`. |
| Student's t quantile | `student_t_quantile` | `P(\|T\| > t) = I_{ν/(ν+t²)}(ν/2, 1/2)`, DLMF §8.18 and Abramowitz & Stegun 26.7.1, inverted by bisection; references from `scripts/incomplete_beta_reference.py`. |
| Short and closest vectors under an integral form | `short_vectors_form`, `closest_vectors_form` | Fincke & Pohst, *Improved methods for calculating vectors of short length in a lattice, including a complexity analysis*, Math. Comp. 44 (1985), 463–471; Schnorr & Euchner, *Lattice basis reduction: improved practical algorithms and solving subset sum problems*, Math. Programming 66 (1994), 181–199 (the enumeration with the zig-zag order). Exact Gram–Schmidt by fraction-free (Bareiss) elimination — Bareiss, *Sylvester's identity and multistep integer-preserving Gaussian elimination*, Math. Comp. 22 (1968), 565–578 — with pruning on outward-rounded enclosures of its rationals, so no vector within the bound is lost to rounding; every returned norm is recomputed exactly. |
