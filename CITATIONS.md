...
| Exact NTT multiplication and squaring | `bigint::ntt::multiply` / `bigint::ntt::square` (dispatched from `BigUint::mul` / `square`) | Schönhage & Strassen, *Schnelle Multiplikation großer Zahlen*, Computing 7 (1971), 281–292, for modular-transform integer multiplication; iterative radix-2 transform after Cooley & Tukey, *An algorithm for the machine calculation of complex Fourier series*, Math. Comp. 19 (1965), 297–301. Rump uses two NTT primes and exact CRT rather than floating-point recovery; squaring pointwise squares one transformed operand. |
