...
| Structured elimination with composition replay | `filter_merge`, `FilteredMatrix`, `minimum_spanning_tree` | Structured Gaussian elimination as NFS filtering practice; modern treatment and the minimum-spanning-tree pivot (least fill-in over a star elimination) in Bouillaguet & Zimmermann, *Parallel Structured Gaussian Elimination for the Number Field Sieve*, J. Math. Cryptol. (2021), §3, after Cavallar; replay by carried row compositions rather than a history log is this crate's choice, proven by the expansion identity in tests. |
