...
arithmetic that sizes it assume a 64-bit \code{usize}. This is enforced rather
than merely documented --- a \code{compile\_error!} rejects any target whose
pointer width is not 64, so a 32-bit build fails with a diagnostic explaining
why instead of compiling and misindexing at run time on an operand large
enough to overflow the product.
...
use rump::{
    ...
    is_probable_prime_bpsw, miller_rabin_with_bases,
    ...
    remove_factor, smooth_parts, mod_sqrt, mod_sqrt_prime_power, valuation,
    SmoothnessBase,
    gauss_reduce_weighted, lll_reduce, lll_reduce_delta,
    BarrettContext, BigInt, BigUint, Gf2m, MontgomeryContext, PolyMod, PolyZ, WordReciprocal,
    RandomSource, Sign,
};
...
let reduced = gauss_reduce_weighted([[1024, 0], [37, 1]], [1, 12]).expect("valid");
