...
\code{roots\_mod\_prime\_power} returns every root of $f$ modulo $p^e$ by
Hensel lifting. Taylor truncated after one term is exact one level up,
\[
  f(r + t p^k) \equiv f(r) + t\,p^k f'(r) \pmod{p^{k+1}},
\]
and writing $f(r) = s\,p^k$ --- legitimate because $r$ is a root modulo
...
root-finder must add to it, and it is why the number of roots modulo $p^e$
is not in general the number modulo $p$ --- the multiple roots, those lying
