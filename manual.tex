...
A sieve working under the skewed metric $(x/\sqrt{s})^2 + (y\sqrt{s})^2$
passes $w = (1, s)$. Scaling a quadratic form by a positive constant changes
neither the comparisons nor the rounding, and scaling by $s$ clears the
square roots, so the whole computation is exact \code{i128}. This is the
substantive difference from a floating-point metric, which loses the ordering
once the weighted coordinates pass $2^{53}$ and degrades quietly to a poor
basis. It panics on a dependent pair, a non-positive weight, or a weighted
norm past \code{i128}.
