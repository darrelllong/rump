...
use rump::{
    ...
    BarrettCtx, BigInt, BigUint, Gf2m, MontgomeryCtx, PolyModP, PolyZ, Reciprocal,
    Rng, Sign,
};
...
\code{Reciprocal::new(d)} precomputes, for a fixed non-zero $d$, the
...
\begin{lstlisting}
let r = Reciprocal::new(NonZeroU64::new(1_000_003).expect("non-zero"));
...
the Montgomery ladder when $n$ is odd, and over a \code{BarrettCtx} when
...
build the \code{BarrettCtx} itself and keep it, since the context costs one
...
\code{SmoothBase::new(P)} is the same computation with $z = \prod P$ built
...
let base = SmoothBase::new(&primes).expect("entries at least two");
