...
This manual tracks the development tree (version 0.3.0, untagged, on
\code{main} above the \code{v0.2.2} tag). The one breaking change since
0.2.2 is that \code{product\_tree} returns a \code{ProductTree} and
\code{remainder\_tree} takes one, in place of the former
\code{Vec<Vec<BigUint>{>}}; \code{CHANGELOG.md} carries the migration note.
Behaviours new since 0.2.1 and absent from
...
fails. The measured shorter-operand crossovers are 65,536 limbs serially,
32,768 with two useful contexts, and 8,192 with four or more, with an
additional padding-efficiency gate because a power-of-two transform doubles
discontinuously.
...
Four regimes, each measured against the kernel it displaces: below 8 limbs
the passes do not repay themselves and the general multiplication is used;
from there to the Karatsuba crossover the schoolbook squaring; from there
to 448 limbs a Karatsuba squaring, whose three sub-products are themselves
squares, since $z_1 = (a_0+a_1)^2 - a_0^2 - a_1^2$; above 448 limbs the
Toom kernels win until NTT admission. NTT squaring uses one transform array,
one forward transform per prime, and pointwise self-products rather than a
general product's duplicate input transform. On the M4 crossover host it is
about 1.47 times faster than general NTT multiplication and uses half the
...
multiply-subtract step's borrow corrects that final case. \code{rem}
...
requires an odd one. \code{BarrettContext::new(n)} returns \code{None} for
...
\code{MontgomeryContext::new(n)} returns \code{None} for even or zero $n$;
...
bare integer --- the earlier API took \code{BigUint} and could only check in
debug builds, so a release build accepted an unencoded or unreduced operand
and returned a wrong answer. The type carries all three instead. Where the
...
sequence instead of allocating per multiply --- measured per-operation at
about 43\% for a 64-bit modulus, roughly 25--33\% at 256 bits, and roughly
20\% at 512, falling to 2--3\% at 2048 bits and to the edge of measurement
($\sim$1\%) at 4096; the in-tree \code{mont\_workspace\_timing} probe
reproduces the numbers, printing its per-pass spread. The exponentiation
ladder has two engines, chosen by exponent width: above 64 bits, left-to-right
$k$-ary with a fixed 4-bit window table held in the domain
(HAC Algorithm~14.82), seeded from the top window; at 64 bits or fewer
(e.g.\ the F4 public exponent 65\,537), right-to-left binary
...
\code{sqrt\_mod(a, p)} returns a square root of $a$ rem an odd prime
...
below 96 coefficients, Karatsuba above, with two further clauses. The
...
when the shorter one barely reaches past it --- so between 96 and 128
coefficients only near-balanced pairs split, and above 128 every ratio
...
of them; each is then emitted $k$ times. Floating point only \emph{locates}
roots whose count is already known.
...
weighted coordinates pass $2^{53}$. It panics on a dependent pair, a
non-positive weight, or arithmetic past \code{i128}; the rounding step needs
twice the norm, so the working bound is that norms fit $2^{126}$.
...
\code{roots\_mod\_prime\_power} & a zero exponent, a base below 2, a polynomial that is zero or has every coefficient divisible by $p^e$, or a lift wider than \code{MAX\_ROOT\_LEVEL} \\
